...
## 🚀 현황 요약 및 핵심 분석 (v0.3.1)

### 1. 주요 기능 및 최근 변경 사항
*   **견적 관리 뷰 개선**: 견적 상세 목록의 정렬 우선순위를 변경하여, 생성 일시 이후 수량(qty) 오름차순 정렬을 추가 적용.
*   **견적 수량 분할 개선**: 여러 수량(예: 1/2/3) 입력 시, 품목이 입력 수량의 오름차순으로 자동으로 등록되도록 수정.
*   **이윤 음수 입력 허용**: 견적 상세 및 수동 입력 필드에서 이윤(%)에 음수(마이너스 마진) 입력이 가능하도록 타입 및 예외 처리 수정.
*   **기초 데이터 관리뷰**: 소재 관리 페이지에서 원자재/후처리/열처리 탭의 데스크톱 사이드 등록 창에 스크롤 고정(`sticky`) 속성을 추가하여 긴 목록 스크롤 시에도 지속 접근이 가능하도록 UI 개선.
...
### 3. 향후 개선 과제
*   **코드 품질 개선 (3단계 계획 중)**: 대형 파일 분리 리팩토링 (`useEstimateLogic.ts`, `ShipmentList.tsx`, `Settings.tsx`), 문서 정합성 수정.
*   **패턴 확산**: `OrderDetail.tsx`의 Hook/Modal 분리 패턴을 타 페이지에도 적용.
*   **재고 관리**: 출하 내역과 연동된 실제 재고 관리 기능 구현 (기획 중).

> **2026-03-05 완료**: 견적 및 소재 관리 UI/정렬 등 버그성 유저인터페이스 개선 사항 반영 후 v0.3.1 정식 릴리즈.
