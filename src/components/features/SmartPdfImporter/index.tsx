  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      if (e.ctrlKey) {
        ...
        const newScale = Math.min(Math.max(scale + delta, 0.5), 5.0);
        setScale(newScale);
      }
    };
          ...
          {!file ? (
            ...
            <>
              <div className="bg-white p-2 border-b flex justify-between items-center z-10 shadow-sm w-full">
                ...
                <div className="flex items-center gap-2">
                   <span className="text-xs font-bold text-blue-600">
                     {isProcessing ? '🔄 분석 중...' : isPanning ? '✋ 이동 모드' : '🖱️ 도번 영역 드래그 (휠: 이동)'}
                   </span>
                   <div className="h-4 w-[1px] bg-slate-300 mx-1"></div>
                   <button onClick={() => setScale(s => Math.max(0.5, s - 0.2))} className="px-2 py-0.5 text-xs border rounded hover:bg-slate-50">－</button>
                   <span className="text-xs font-mono w-12 text-center">{Math.round(scale * 100)}%</span>
                   <button onClick={() => setScale(s => Math.min(5.0, s + 0.2))} className="px-2 py-0.5 text-xs border rounded hover:bg-slate-50">＋</button>
                </div>
              </div>
              ...
              >
                <div 
                  ref={pdfWrapperRef}
                  className={`relative shadow-2xl transition-transform duration-100 ease-out origin-top-center select-none bg-white ${
                ...
                >
                  ...
                  {selection.w > 0 && (
                    ...
                    >
                       <div className="absolute -top-6 left-0 bg-red-600 text-white text-[10px] px-1 py-0.5 font-bold whitespace-nowrap">
                         인식 중...
                       </div>
                    </div>
                  )}
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-2">
            {ocrResults.map((res, idx) => (
              <div 
                ...
                className={`flex flex-col gap-2 p-2 border rounded cursor-pointer transition-all ${
                  pageNumber === res.page ? 'ring-2 ring-blue-500 bg-blue-50 border-blue-200' : 'bg-white hover:bg-slate-50 border-slate-200'
                } ${res.skip ? 'opacity-50' : ''}`}
              >
