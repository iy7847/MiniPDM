...
## 🛠️ Slash Commands & Specialized Tasks

- **`/release` 또는 `/릴리즈`**: 배포 완료 후 릴리즈 노트를 작성합니다.
  - **형식**: 반드시 복사가 용이하도록 **`text` 코드 블록**으로 제공하십시오.
  - **양식 지침**:
    1. 제목: `🚀 [v X.X.X] 업데이트 내용`
    2. 대항목: `1. 기능명 (English Name)`
    3. 세부항목: 불렛 포인트(`*`)를 사용한 구체적 기술 (예: `* 세부내용: 설명`)
    4. 내용 구성: 주요 기능 수정, UI/UX 개선, 시스템 최적화 순서로 정리
